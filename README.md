...
1. Build image
```bash
docker build -t mwml-httynn-app:latest -f Dockerfile .
```
2. Run container
```bash
docker run -d -p 5000:5000 -p 6006:6006 --name mwml-httynn-app mwml-httynn-app:latest
```
...
```
python mwml-httynn-app/train.py --overfit
```
...
```
docker run -p 5000:5000 --name mwml-httynn-app mwml-httynn-app:latest
```

• Get inside container if using `CMD ["/bin/bash"]`
```
docker run -p 5000:5000 -it mwml-httynn-app /bin/bash
```
